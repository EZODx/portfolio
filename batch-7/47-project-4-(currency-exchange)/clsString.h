...
public
	...
	static short CountWords(string theString)
	{
		...
		while ((pos = theString.find(delime)) != stdstringnpos)
		{
	...
	static short CountLetters(string theString, enWhatToCount whatToCount = enWhatToCountAll)
	{

		if (whatToCount == enWhatToCountAll)
		{
			return theString.length();
		}
		...
		for (int i = 0; i  theString.length(); i++)
		{
			...
			if (islower(theString[i]) && whatToCount == enWhatToCountSmallLetters)
				counter++;
			else if (isupper(theString[i]) && whatToCount == enWhatToCountCapitalLetters)
				counter++;
	...
	static vectorstring Split(string S1, string delim)
	{
		...
		while ((pos = S1.find(delim)) != stdstringnpos)
		{
	...
	static string ReverseWordsInString(string str)
	{
		...
		vector stringiterator iter = vString.end();
